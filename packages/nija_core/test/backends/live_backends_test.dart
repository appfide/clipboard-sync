...
///
/// ```sh
/// NIJA_TEST_SUPABASE_URL=https://xxx.supabase.co \
/// NIJA_TEST_SUPABASE_ANON_KEY=... dart test -t live
/// ```
///
...
void main() {
  ...
  BackendConfig? cfg(String id, Map<String, String> keys) {
    ...
    for (final e in keys.entries) {
      final v = env['NIJA_TEST_${id.toUpperCase()}_${e.key.toUpperCase()}'];
  ...
  for (final e in specs.entries) {
    ...
    if (c == null) {
      test(
        ...
        () {},
        skip: 'set NIJA_TEST_${e.key.toUpperCase()}_* to run',
      );
      continue;
    }
