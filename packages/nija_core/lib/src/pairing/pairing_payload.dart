...
class PairingPayload {
  ...
  const PairingPayload({
    ...
    this.passphraseDelivered = false,
  ...
  factory PairingPayload.fromJson(Map<String, Object?> j) {
    ...
    return PairingPayload(
      ...
      deviceKeys: j['k'] as String?,
      adminPub: j['ap'] as String?,
      adminKey: j['ak'] as String?,
      encryption: j['x'] == true,
      passphraseDelivered: j['pd'] == true,
  ...
  /// The joining device's identity (`DeviceKeys.encode`), generated by the
  /// host so the host can sign its public keys before it ever connects.
  final String? deviceKeys;

  /// Admin public key to pin (base64). `null` for legacy groups.
  final String? adminPub;

  /// Admin private seed (`AdminKey.encode`) when the host grants the new
  /// device the right to manage the group.
  final String? adminKey;

  /// Whether the group encrypts content.
  final bool encryption;

  /// Whether the host sealed the current passphrase into the new device's
  /// row (it arrives through the key envelope on first check-in). When
  /// false and [encryption] is true, the joining device must ask for it.
  final bool passphraseDelivered;

  /// Whether the joiner still has to type the passphrase.
  bool get needsPassphrase => encryption && !passphraseDelivered;

  /// Whether the code pins an admin key (signed group).
  bool get isSignedGroup => adminPub != null;
  ...
  Map<String, Object?> toJson() => <String, Object?>{
    ...
    'h': hostDeviceId,
    'hn': hostDeviceName,
    if (deviceKeys != null) 'k': deviceKeys,
    if (adminPub != null) 'ap': adminPub,
    if (adminKey != null) 'ak': adminKey,
    if (encryption) 'x': true,
    if (passphraseDelivered) 'pd': true,
    'r': role.wire,
...
///
/// Wire format: `NIJA1.` + base64url(JSON `{s, n, c}`) where
...
abstract final class PairingCodec {
  ...
  static const String prefix = 'NIJA1.';
  ...
  static Future<PairingPayload> open(
    ...
    if (!trimmed.startsWith(prefix)) {
      throw PairingException('That is not a Nija pairing code.');
    }
