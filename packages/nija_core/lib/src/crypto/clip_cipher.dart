...
import 'package:nija_core/src/util/hashing.dart';
...
class ClipCipher {
  ...
  static Future<ClipCipher> fromPassphrase(
    ...
    // Frozen wire constant. The v1 label feeds the salt, so renaming it
    // derives a different key and makes every clip already encrypted in a
    // user's database undecryptable. The app's name is not part of its
    // key schedule; only the version is.
  ...
  Future<String> contentFingerprint(String content) async {
    ...
    // Frozen wire constant: the v1 label is baked into every fingerprint
    // already stored in a user's database, which outlives an app reinstall.
    // Renaming the app must not silently break dedup against those rows.
